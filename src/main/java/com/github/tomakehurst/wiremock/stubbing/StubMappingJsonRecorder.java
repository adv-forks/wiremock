...
public class StubMappingJsonRecorder implements RequestListener {
    ...
    private void writeToMappingAndBodyFile(Request request, Response response, RequestPattern requestPattern) {
        ...
        HttpHeader timing = response.getHeaders().getHeader("Timing");
        if (timing.isPresent() && !timing.values().isEmpty()) {
            responseDefinitionBuilder.withFixedDelay(Integer.parseInt(timing.values().get(0)));
        }
